...
class Set_Time(tk.Toplevel):
    def __init__(self, title="Set Time"):
        super().__init__()
        self.title(title)
        
        style = ttk.Style(self)
        
        style.configure("mystyle.TButton", highlightthickness=0, bd=0, font=('Arial', 24))
        style.configure("mystyle.TCombobox.Heading", font=('Arial', 48))
        ...
        hours_combo = ttk.Combobox(self, width=2, values=tuple(range(0,100)),
                                   textvariable=self.hours_str, style="mystyle.TCombobox")
        minutes_combo = ttk.Combobox(self, width=2, values=tuple(range(0,60)),
                                   textvariable=self.mins_str, style="mystyle.TCombobox")
        seconds_combo = ttk.Combobox(self, width=2, values=tuple(range(0,60)),
                                   textvariable=self.secs_str, style="mystyle.TCombobox")
        ...
        colon_label = ttk.Label(self, text=":")#, font=time_font)
        colon_label2 = ttk.Label(self, text=":")#, font=time_font)
        ...
        set_time_button.grid(row=2, column=3, columnspan=2, padx=5, pady=5)
